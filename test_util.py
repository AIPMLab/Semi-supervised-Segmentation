...
def test_all_case(vnet, resnet, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4,
                  save_result=True, test_save_path=None, preproc_fn=None, dataset_pan=False):
    total_metric = 0.0
    for image_path in tqdm(image_list):
        ...
        prediction, score_map = test_single_case(vnet, resnet, image, stride_xy, stride_z, patch_size,
...
def test_all_casepan(vnet, resnet, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4,
                     save_result=True, test_save_path=None, preproc_fn=None, dataset_pan=False):
    total_metric = 0.0
    for image_path in tqdm(image_list):
        # 提取文件名的正确方式
        if dataset_pan:
            id = os.path.basename(image_path)[:-3]  # 去掉扩展名 .h5
        else:
            id = os.path.basename(os.path.dirname(image_path))  # 根据路径结构提取 id
        ...
        # 保存结果时使用 os.path.join 拼接路径，避免路径错误
        if save_result:
            ...
            nib.save(nib.Nifti1Image(image[:].astype(np.float32), np.eye(4)),
                     os.path.join(test_save_path, f"{id}_img.nii.gz"))
            nib.save(nib.Nifti1Image(label[:].astype(np.float32), np.eye(4)),
                     os.path.join(test_save_path, f"{id}_gt.nii.gz"))

    avg_metric = total_metric / len(image_list)
    print(f'average metric is {avg_metric}')
